...
fails).
 ...
 A rule is any object derived from Symbol that has the following method on it:
        static shared_ptr<Symbol> TryParse(shared_ptr<Lexer> lexer, const string
...
public:
  ...
  void AddSubsymbol(vector<shared_ptr<Symbol>>::iterator begin,
                    vector<shared_ptr<Symbol>>::iterator end) {
    for_each(begin, end, [&](shared_ptr<Symbol> &symbol) {
      FailFastAssert(symbol != nullptr);
  ...
  virtual void AddToStream(stringstream &stream) {
    for_each(
        m_subSymbols.begin(), m_subSymbols.end(),
        [&stream](shared_ptr<Symbol> &symbol) { symbol->AddToStream(stream); });
  }

  const vector<shared_ptr<Symbol>> &children() { return m_subSymbols; }
...
public:
  ...
  static shared_ptr<Symbol> TryParse(shared_ptr<Lexer> lexer,
...
public:
  ...
  static shared_ptr<Symbol> TryParse(int symbolIndex, shared_ptr<Lexer> lexer,
                                     const string &errorMessage) {
    switch (symbolIndex) {
    case 0:
...
public:
  ...
  static shared_ptr<Symbol> TryParse(shared_ptr<Lexer> lexer,
    ...
    for (int symbolIndex = 0; symbolIndex < argsCount; ++symbolIndex) {
      shared_ptr<Symbol> streamSymbol = Args::TryParse(
...
public:
  ...
  static bool CheckTree(vector<shared_ptr<Symbol>> &tree, int level0Index,
  ...
  // consumed (requireEOF)
  template <class rule>
  static shared_ptr<Symbol> TestTryParse(string testString, int &deepestFailure,
                                         string &errorMessage,
  ...
  static bool
  CheckFlattenedSingleSymbolResult(shared_ptr<Symbol> rule, unsigned short ID,
                                   const string &result,
  ...
  static string PrintTree(vector<shared_ptr<Symbol>> &symbols) {
    stringstream stream;
    for_each(symbols.begin(), symbols.end(), [&](shared_ptr<Symbol> symbol) {
